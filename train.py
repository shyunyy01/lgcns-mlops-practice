...
sys.excepthook = handle_exception  # 전체 코드에 대해서 에러를 잡고 싶을 때(logger.py)
...
if __name__ == "__main__":
    ...
    X.assign(rent=y).to_csv(
        ...
        os.path.join(DATA_PATH, "storage", "housr_rent_train_features.csv"),
        index=False,
    )
    ...
    experiment_name = "new_experiment_with_log"  ## 소현: 로그 수정 후
    ...
    for i, params in enumerate(param_set):
        ...
        with mlflow.start_run(run_name=f"Run {i}"):
            ...
            name = regr.__class__.__name__  ## 소현: GradientBoostingRegressor
